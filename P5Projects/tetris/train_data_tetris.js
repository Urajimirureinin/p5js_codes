...
function keyPressed() {
    if (keyIsDown(SHIFT)) {
        ...
        switch (keyCode) {
            case LEFT_ARROW:
                if (game.mino.valid(-1, 0, game.field)) {
                    game.mino.x--;
                    pressed_key=37;
                }
                break;
            case RIGHT_ARROW:
                ...
                break;
            case DOWN_ARROW:
                if (game.mino.valid(0, 1, game.field)) {
                    game.mino.y++;
                    pressed_key=40;
                }
                break;
    ...
    // 新しいコード: テキストエリアにデータを追加
    const blockMap = game.getFixedBlockMap();
    const data = {
        input: blockMap,
        output: { [pressed_key] : 1}
    };
...
function appendToTextArea(data) {
    ...
    const currentText = logArea.value;

    // 新しいデータをJSON文字列に変換して追加
    const newText = JSON.stringify(data, null, 2);
    logArea.value = currentText+ "," + '\n' + newText;
}